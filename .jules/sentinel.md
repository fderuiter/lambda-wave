...
## 2026-06-02 - [Risk Level: HIGH] **Vector:** src/Hardware/Consumer.hs **Hazard:** Denial of Service (DoS) / Allocation Loop
The `parseTLVs` function trusted the `tlvLen` field from the sensor data. A corrupted or malicious packet could specify a huge `tlvLen` (e.g., 0xFFFFFFFF), causing the parser to attempt to read millions of points. This would result in an infinite loop (waiting for data) or massive memory allocation attempts, hanging the consumer thread and freezing the safety system.
**Fix:** Introduced `maxTLVSize` (65536) and added explicit checks in `parseTLVs`. Packets exceeding this size are immediately rejected as corrupt, allowing the consumer to skip the garbage and recover.
