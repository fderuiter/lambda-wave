...
import Foreign.ForeignPtr (ForeignPtr, castForeignPtr, touchForeignPtr, withForeignPtr)
      ...
      case BL.elemIndex 1 bs of
        ...
        Just idx ->
          let candidate = BL.drop idx bs
           in -- ⚡ Bolt Optimization: Replace `BL.length candidate < 8` with `BL.length (BL.take 8 candidate) < 8`
              -- to avoid forcing an O(N) evaluation of the entire chunk chain just to check length. Restores O(1) streaming performance.
              if BL.isPrefixOf magicPattern candidate || BL.length (BL.take 8 candidate) < 8
                then (acc + idx, candidate) -- Found match or keep partial match
                else -- Found 0x01 but not followed by correct sequence (Garbage)
...
toPoint3D mountingOffset Point {..} =
  Point3D
    { px = float2Double px',
      py = float2Double py',
