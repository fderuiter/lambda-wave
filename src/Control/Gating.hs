...
import SignalProcessing.Kalman (KalmanConfig (..), KalmanState (..), predict, update, pattern V3)
...
processFrame translations stateVar frame = do
  ...
  (finalBeamState, hardwareUpdateNeeded, mEvtToLog, mAudioCmd) <- atomically $ do
    ...
    let mAudio =
          if changed && audioAlertEnabled s
