...
RingBufferControl *create_ring_buffer(size_t size) {
  ...
  // Relaxed ordering is sufficient for initialization as the pointer is not yet
  // published
...
ssize_t read_from_uart(RingBufferControl *handle, int uart_fd) {
  ...
  // Load read_offset acquire: ensures we see the latest value updated by the
  // consumer
  ...
  if (read_offset > current_offset) {
    // Space is between write cursor and read cursor (minus 1 to distinguish
    // full vs empty)
    ...
    // ...unless read_offset is 0, in which case we must stop one byte short of
    // the end to avoid wrapping to 0 and colliding with read_offset (making it
    // look empty).
    if (read_offset == 0) {
...
size_t get_write_offset(RingBufferControl *handle) {
  if (!handle)
    return 0;
  // Acquire semantics to ensure we see the data committed before this offset
  // update
