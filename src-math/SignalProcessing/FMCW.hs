...
import SignalProcessing.Matrix (dotGen, normSq, scaleAndAddV, scaleV, subV)
...
data CZTParams = CZTParams
  { -- | f_0: Start frequency of the zoom window (Hz)
    cztStartFreq :: Double,
    -- | B_zoom: Bandwidth of the zoom window (Hz)
    cztBandwidth :: Double,
    -- | K: Number of frequency steps in the output
    cztSteps :: Int,
    -- | f_s: Sampling rate of the IF signal (Hz)
...
chirpZTransform params x_n = map calculateBin [0 .. cztSteps params - 1]
  where
    ...
    calculateBin k =
      let k_idx = fromIntegral k :: Double
          -- f_k = f_0 + B_zoom * (k / K)
          ...
          !w = cis theta_step
       in -- Summation: sum(x[n] * exp(i * theta_step * n))
...
-- Safety: Total function, handles all inputs gracefully.
calculateDisplacement ::
  -- | f_min: Start frequency of the chirp (Hz) (e.g. 77e9)
  Frequency ->
...
data MTIConfig = MTIConfig
  { -- | Base learning rate (standard motion)
    mtiAlphaBase :: !Double,
    -- | Max learning rate (static environment)
    mtiAlphaMax :: !Double,
  ...
  if null prevMean
    then goInit input
      ...
      let !motionMetric = calculateMotionMetric prevMean input
          !alpha =
            if motionMetric < mtiThreshold config
