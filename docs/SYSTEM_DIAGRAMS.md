...
graph TB
    ...
    WD -.->|Monitors| GL
    WD -.->|Monitors| FMCW
    GL -->|AuditEvent| AQ((Audit Queue Boundary))
    AQ --> AL
    GPIO -->|Beam Status| AQ
...
The central system state and the immutable audit trail for compliance, showing the decoupled management of logs.
...
classDiagram
    ...
    class AuditLoggerSubsystem {
        <<Thread>>
        +encryptLog()
        +rotateLog()
    }
    SystemState ..> AuditEvent : produces
    AuditLoggerSubsystem ..> AuditEvent : consumes from shared queue via boundary
```
...
### 3.7 Safety: Token Authentication

The token system ensures that only authorized entities can perform sensitive operations.

```mermaid
sequenceDiagram
    participant U as User/UI
    participant S as Safety.Token
    participant K as Crypto Key

    U->>S: Request Token (User ID)
    S->>K: Sign User ID + Timestamp
    K-->>S: HMAC Signature
    S-->>U: Secure Token

    U->>S: Validate Token (Token)
    S->>S: Parse ID & Timestamp
    S->>S: Check Expiry (< 1 hour)
    S->>K: Re-verify Signature
    alt Valid
        S-->>U: Authorized
    else Invalid
        S-->>U: Access Denied
...
### 3.8 Data: Internationalization (I18n)

Lambda-Wave supports multiple languages for UI and audit logging.

```mermaid
graph LR
    L[locales.json] --> P[I18n Parser]
    P --> M[Translation Map]
    M --> T[translate function]

    S[System State: lang] --> T
    K[Key: BEAM_ON] --> T
    T --> R[Localized String: "Beam ON"]
```

### 3.9 Control: WebUI WebSocket Bridge

The WebUI provides a remote dashboard via WebSockets.

```mermaid
graph TD
    H[Haskell RTS] -->|STM TVar| W[WebUI Thread]
    W -->|JSON Encode| WS[WebSocket Server]
    WS -->|Encrypted Stream| C[Browser Client]
    C -->|Commands| WS
    WS -->|Auth Check| H
```
