...
import Numeric.Kinematics (Meters (..), MetersPerSecond (..), MetersPerSecondSquared (..), Millimeters (..), MillimetersPerSecond (..), MillimetersPerSecondSquared (..), mmPerS2ToMetersPerS2, mmPerSToMetersPerS, mmToMeters)
...
scaleKalmanStateToMeters ks =
  let (pos, vel, acc) = case x ks of
   ...
   in ks
        { x =
            V3
              (let Meters m = mmToMeters (Millimeters pos) in m)
