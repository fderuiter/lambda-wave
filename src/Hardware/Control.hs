...
import Hardware.Manifest (configBaudRate, logicPin, watchdogPin)
...
openConfigPort :: FilePath -> IO Fd
openConfigPort path = openFd path ReadWrite defaultFileFlags
#else
openConfigPort :: FilePath -> IO Fd
openConfigPort path = openFd path ReadWrite Nothing defaultFileFlags
#endif
      ...
      | otherwise -> do
          ...
          if "Done" `B.isInfixOf` newAcc
            then return True
              ...
              if "Error" `B.isInfixOf` newAcc || B.length newAcc > 4096
                then return False
