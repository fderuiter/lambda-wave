...
## 2026-01-28 - Aria-Pressed Anti-Pattern on Dynamic Labels
**Learning:** Using `aria-pressed` on a button that simultaneously changes its visible text label (e.g., 'Pause' changing to 'Resume') creates a confusing and redundant experience for screen reader users (e.g., announcing "Resume, toggle button, pressed").
**Action:** Use `aria-pressed` ONLY for toggle buttons with persistent labels. If the button's action text changes to reflect the new state, remove the `aria-pressed` attribute entirely so it is treated as a standard action button.
