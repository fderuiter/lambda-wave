...
extern "C" void start_cpp_hud_loop(void) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!logged_in) {
            ...
            if (ImGui::Button("EN")) g_active_language = "en";
            ImGui::SameLine();
            if (ImGui::Button("ES")) g_active_language = "es";
            ImGui::SameLine();
            if (ImGui::Button("FR")) g_active_language = "fr";

            ImGui::Text("Calibration Status: %s", g_calibration_status == 1 ? "Valid" : "Invalid");
            
            ImVec4 beamColor;
            if (g_beam_state == 1) beamColor = ImVec4(0, 1, 0, 1); // BeamOn
            else if (g_beam_state == 2) beamColor = ImVec4(1, 1, 0, 1); // BeamHold
            else beamColor = ImVec4(1, 0, 0, 1); // BeamOff
            
            ImGui::TextColored(beamColor, "STATUS: %s", g_localized_beam_state.c_str());
            
            // Respiratory trace
            std::vector<float> trace(g_resp_history.begin(), g_resp_history.end());
            ImGui::PlotLines("Respiratory Trace", trace.data(), trace.size(), 0, NULL, -20.0f, 20.0f, ImVec2(0, 100));
