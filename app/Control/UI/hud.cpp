...
void display() {
    ...
    if (!g_loggedIn) {
        ...
        ImGui::Text(g_language == 0 ? "Real-time Telemetry" : (g_language == 1 ? "Telemetria en tiempo real" : "Telemetrie en temps reel"));
        
        g_traceHistory.push_back(g_respiratoryTrace);
        if (g_traceHistory.size() > 100) g_traceHistory.erase(g_traceHistory.begin());
        ImGui::PlotLines("Respiratory Trace", g_traceHistory.data(), g_traceHistory.size());

        ImGui::Text("Point Cloud Renderer (OpenGL)");
        // The actual 3D point cloud would be rendered via pure OpenGL outside ImGui,
        // or rendered to a framebuffer and displayed as an ImGui image.
        ImGui::End();
    }
