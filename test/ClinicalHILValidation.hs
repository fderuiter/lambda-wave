...
import Data.Config (gatingTolerance, targetHeight)
import qualified Data.HashMap.Strict as HM
import Data.List (sort)
import qualified Data.Map.Strict as Map
import Data.Time.HighRes (getMonotonicTimeNS)
import Data.Types
import Hardware.Control (GpioChannel (..), initGpio, readBeamChannel, setupWatchdog)
...
runHILSimulation name rig duration = do
  ...
  let breaches =
        [ (t, truePos, bState) | (t, truePos, _estPos, bState, _, _) <- results, abs (truePos - targetHeight) > (gatingTolerance + 1.0) && bState == BeamOn
        ]
