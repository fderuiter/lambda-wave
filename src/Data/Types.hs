...
instance NFData AuditEvent where
  rnf (AuditEvent t s src msg) = rnf t `seq` rnf s `seq` rnf src `seq` rnf msg
...
data SystemState = SystemState
  ...
  , auditQueue :: TBQueue AuditEvent -- ^ Queue for audit logging
  }

instance NFData SystemState where
  rnf (SystemState pts bs t iso hb ks aq) =
      rnf pts `seq` rnf bs `seq` rnf t `seq` rnf iso `seq` rnf hb `seq` rnf ks `seq` aq `seq` ()
