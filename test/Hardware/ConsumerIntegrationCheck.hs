...
main = do
  ...
  if "synthetic_capture.bin" `isSuffixOf` filePath
    then verifySyntheticSimple frames
      ...
      if "complex_capture.bin" `isSuffixOf` filePath
      ...
      | otherwise =
          let (frames, consumed, maybeErr) = parseStream 0.0 input
           ...
           in if consumed == 0 && not (BL.null input)
                then -- Stuck? Force advance by 1 byte to avoid infinite loop
                ...
                else go newFrames newBytes newCorruption (BL.drop consumed input)
