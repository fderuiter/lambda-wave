...
//  IMGUI_ENABLE_FREETYPE_LUNASVG' (#6591) 2023/01/04: fixed a packing issue
//  which in some occurrences would prevent large amount of glyphs from being
//  packed correctly. 2021/08/23: fixed crash when FT_Render_Glyph() fails to
//  render a glyph and returns NULL. 2021/03/05: added
...
//  prefered texture format. 2021/01/28: added support for color-layered glyphs
...
struct FreeTypeFont {
  bool InitFont(
      FT_Library ft_library, const ImFontConfig& cfg,
      unsigned int
  ...
  for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
    ...
    for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++) {
      ...
      if (atlas->TexPixelsAlpha8 != nullptr) {
        ...
        for (int y = 0; y < info.Height;
             y++, blit_dst += blit_dst_stride, blit_src += blit_src_stride)
          for (int x = 0; x < info.Width; x++) blit_dst[x] = blit_src[x];
      }
    }
...
static void ImGuiLunasvgPortFree(FT_Pointer* _state) { IM_DELETE(*_state); }
